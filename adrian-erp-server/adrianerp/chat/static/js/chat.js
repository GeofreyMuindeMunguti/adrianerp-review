...
const userDiv = (senderId, receiverId, name, online) =>
    (`<a href="/chat/${senderId}/${receiverId}" id="user${receiverId}" class="collection-item row">
                    <img  src="/static/images/avatar.png" alt="" class="circle responsive-img col s2">
...
function send(sender, receiver, message) {
    $.post('/api/chat/messages', '{"sender": "' + sender + '", "receiver": "' + receiver + '","message": "' + message + '" }', function (data) {
        console.log(data);
...
function receive() {
    $.get('/api/chat/messages/' + sender_id + '/' + receiver_id, function (data) {
        console.log(data);
        if (data.length !== 0) {
...
function getUsers(senderId, callback) {
    return $.get('/api/chat/users', function (data) {
